...
with tab_water:
    ...
    with col_inputs[0]:
        input_temp = st.number_input("Temperature (°C)", min_value=10.0, max_value=45.0, value=25.0)
    with col_inputs[1]:
        input_ph = st.slider("pH Factor", min_value=0.0, max_value=14.0, value=7.2, step=0.1)
    with col_inputs[2]:
        input_do = st.number_input("Oxygen (mg/L)", min_value=0.0, max_value=20.0, value=6.5)
    with col_inputs[3]:
        input_bod = st.number_input("BOD Level (mg/L)", min_value=0.0, max_value=40.0, value=2.0)
    with col_inputs[4]:
        input_nitrate = st.number_input("Nitrate Level (mg/L)", min_value=0.0, max_value=50.0, value=1.5)
        
    if st.button("🌱 Evaluate Water Safety Index", use_container_width=True):
...
with tab_energy:
    ...
    with col_e_2:
        ...
        pred_year = st.slider("Select Forecast Target Year:", min_value=2025, max_value=2050, value=2032, step=1)
        ...
        user_consumption = st.number_input("Enter localized grid context (GWh):", value=1350000)
